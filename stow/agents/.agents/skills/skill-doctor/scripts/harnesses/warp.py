...
class WarpHarness(HarnessPlugin):
    id = "warp"
    display_name = "Warp"

    def detect_runtime(self, environ=None) -> bool:
        env = environ if environ is not None else os.environ
        if env.get("WARP_IS_LOCAL_MODE_ENABLED") or env.get("WARP_SESSION_ID"):
            return True
    ...
    def is_available(self, ctx: CollectContext) -> bool:
        return bool(self._databases(ctx))

    def missing_source_error(self, ctx: CollectContext) -> str:
        return "error: no Warp conversation databases found"

    def _databases(self, ctx: CollectContext):
        extra = ctx.extra
        return discover_warp_databases(extra.get("warp_db") or (), extra.get("warp_data_dir"))

    def list_sessions(self, ctx: CollectContext) -> List[SessionRef]:
        databases = self._databases(ctx)
        records, scanned = find_warp_conversations(databases, ctx.cutoff)
        refs = []
        for record in records:
            refs.append(SessionRef(
                ...
                file_label=(
                    f"{record['database']}#agent_conversations/"
                    f"{record['conversation_id']}"
                ),
                extra={"record": record, "scanned": scanned, "databases": databases},
            ))
        if refs:
            refs[0].extra["scanned_total"] = scanned
            refs[0].extra["databases"] = databases
        elif databases:
            # Preserve scanned count even when no records survive the cutoff.
            ctx.extra["_warp_scanned"] = scanned
            ctx.extra["_warp_databases"] = databases
        return refs

    def parse_session(self, ref: SessionRef, skill_names, include_subagents: bool):
        return parse_warp_conversation(ref.extra["record"], skill_names, include_subagents)

    def source_info(self, ctx: CollectContext, refs):
        databases = []
        scanned = ctx.extra.get("_warp_scanned", 0)
        if refs:
            databases = refs[0].extra.get("databases") or []
            scanned = refs[0].extra.get("scanned_total", len(refs))
        elif ctx.extra.get("_warp_databases"):
            databases = ctx.extra["_warp_databases"]
        return {
            "databases": [str(path) for path in databases],
            "records_in_window": scanned,
            "records_after_channel_deduplication": len(refs),
        }
