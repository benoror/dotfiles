...
class ClaudePlugin(BasePlugin):
    ...
    def source_available(self, args) -> bool:
        return (Path(args.claude_home).expanduser() / "projects").is_dir()

    def collect(self, args, cutoff, skill_names):
        claude_home = Path(args.claude_home).expanduser()
        files = find_claude_session_files(claude_home, cutoff, args.include_subagents)
        sessions = []
        for mtime, path in files:
            parsed = parse_claude_session(path, skill_names, args.include_subagents)
            if parsed is None:
                continue
        ...
        return sessions, {"home": str(claude_home), "records_in_window": len(files)}

    def global_skill_roots(self, args):
        return [Path.home() / ".claude" / "skills"]

    def missing_error(self, args) -> str:
        home = Path(args.claude_home).expanduser() / "projects"
        return f"error: Claude Code project history not found at {home}"
